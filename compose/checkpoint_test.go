...
func TestSimpleCheckPoint(t *testing.T) {
	...
	result, err := r.Invoke(ctx, "start", WithCheckPointID("1"), WithStateModifier(func(ctx context.Context, path []string, state any) error {
	...
	streamResult, err := r.Stream(ctx, "start", WithCheckPointID("2"), WithStateModifier(func(ctx context.Context, path []string, state any) error {
		assert.Equal(t, 0, len(path))
		state.(*testStruct).A = "state"
		return nil
	}))
...
func TestCustomStructInAny(t *testing.T) {
	GenericRegisterType[testStruct]("test_struct")
	store := newInMemoryStore()
	g := NewGraph[string, string](WithGenLocalState(func(ctx context.Context) (state *testStruct) {
		return &testStruct{A: ""}
	}))
...
func TestNestedSubGraph(t *testing.T) {
	...
	times := 0
	_, err = r.Invoke(ctx, "start", WithCheckPointID("1"), WithStateModifier(func(ctx context.Context, path []string, state any) error {
		assert.Equal(t, 1, len(path))
		state.(*testStruct).A = "state"
		return nil
	}))
...
3`, result)
	...
	times = 0
	_, err = r.Stream(ctx, "start", WithCheckPointID("2"), WithStateModifier(func(ctx context.Context, path []string, state any) error {
		assert.Equal(t, 1, len(path))
		state.(*testStruct).A = "state"
		return nil
	}))
